...
export default function SearchAddress() {
  ...
  const { location, isLocationPermissionGranted, setLocationAndAddress } = useStore()
  ...
  const onLocationSelection = (location: any) => {
    ...
    getPlaceDetails(location.place_id).then((data) => {
    ...
    <SafeAreaView>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <View className="h-full">
          ...
          <TouchableOpacity
            className={classNames("mt-[15px] bg-card h-[46px] w-full justify-self-end absolute bottom-0")}
